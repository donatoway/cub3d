...
unsigned char	*create_file_header(t_game *game, int pad)
{
	...
	bpp = BYTES_PER_PIX;
	file_size = 54 + (bpp * ((int)game->width + pad) *
		(int)game->height);
...
int				write_colors(t_game *game, int fd, int height, int width)
{
	...
	while (i < (int)game->height)
	{
		...
		{
			rgb[0] = ((game->img->img_data[(height - i) * width + j]) >> 16);
			rgb[1] = ((game->img->img_data[(height - i) * width + j]) >> 8);
			rgb[2] = (game->img->img_data[(height - i) * width + j]);
